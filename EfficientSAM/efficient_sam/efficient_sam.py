...
class EfficientSam(nn.Module):
    ...
    def __init__(
        ...
        Arguments:
          ...
          prompt_encoder (PromptEncoder): Encodes input prompts.
          mask_decoder (MaskDecoder): Predicts masks from image and prompts.
          pixel_mean (list(float)): Mean values for image normalization.
          pixel_std (list(float)): Std values for image normalization.
        """
        super().__init__()
    ...
    def _predict_masks_internal(
        ...
        low_res_semantic = None
        if return_boundary and return_semantic:
            low_res_masks, iou_predictions, low_res_boundaries, low_res_semantic = decoder_outputs
            if low_res_boundaries is None:
                low_res_boundaries = torch.zeros_like(low_res_masks)
        elif return_boundary:
            low_res_masks, iou_predictions, low_res_boundaries = decoder_outputs
            if low_res_boundaries is None:
                low_res_boundaries = torch.zeros_like(low_res_masks)
        elif return_semantic:
            low_res_masks, iou_predictions, low_res_semantic = decoder_outputs
            low_res_boundaries = None
        else:
        ...
        if output_w > 0 and output_h > 0:
            ...
            if return_boundary and low_res_boundaries is not None:
                output_boundaries = F.interpolate(
                    low_res_boundaries,
                    (output_h, output_w),
                    mode="bilinear",
                    align_corners=False,
                )
                output_boundaries = torch.reshape(
                    output_boundaries,
                    (batch_size, max_num_queries, num_predictions, output_h, output_w),
                )
            else:
                output_boundaries = None
            if return_semantic and low_res_semantic is not None:
                output_semantic = F.interpolate(
                    low_res_semantic,
                    (output_h, output_w),
                    mode="bilinear",
                    align_corners=False,
                )
                output_semantic = torch.reshape(
                    output_semantic,
                    (
                        batch_size,
                        max_num_queries,
                        output_semantic.shape[1],
                        output_h,
                        output_w,
                    ),
                )
        ...
        if return_boundary and return_semantic:
            return output_masks, iou_predictions, output_boundaries, output_semantic
        if return_boundary:
            return output_masks, iou_predictions, output_boundaries
        if return_semantic:
            return output_masks, iou_predictions, output_semantic
    ...
    def predict_masks(
        ...
        Returns:
          low_res_mask: [B, max_num_queries, num_predictions, H, W]
          iou_predictions: [B, max_num_queries, num_predictions]
        """
        return self._predict_masks_internal(
    ...
    def predict_masks_with_boundary(
        self,
        image_embeddings: torch.Tensor,
        batched_points: torch.Tensor,
        batched_point_labels: torch.Tensor,
        ...
        """
        Predict masks and boundary maps.

        Returns:
          masks: [B, max_num_queries, num_predictions, H, W]
          iou_predictions: [B, max_num_queries, num_predictions]
          boundary_maps: [B, max_num_queries, num_predictions, H, W]
        """
        output_masks, iou_predictions, boundary_maps = self._predict_masks_internal(
            image_embeddings=image_embeddings,
            batched_points=batched_points,
            batched_point_labels=batched_point_labels,
            multimask_output=multimask_output,
        ...
        if boundary_maps is None:
            boundary_maps = torch.zeros_like(output_masks)
        return output_masks, iou_predictions, boundary_maps

    def predict_masks_with_boundary_and_semantics(
        self,
        image_embeddings: torch.Tensor,
        batched_points: torch.Tensor,
        batched_point_labels: torch.Tensor,
        ...
        """
        Predict masks, boundaries, and semantic logits.

        Returns:
          masks: [B, max_num_queries, num_predictions, H, W]
          iou_predictions: [B, max_num_queries, num_predictions]
          boundary_maps: [B, max_num_queries, num_predictions, H, W]
          semantic_logits: [B, max_num_queries, num_classes, H, W]
        """
        output_masks, iou_predictions, boundary_maps, semantic_logits = self._predict_masks_internal(
            image_embeddings=image_embeddings,
            batched_points=batched_points,
            batched_point_labels=batched_point_labels,
            multimask_output=multimask_output,
        ...
        if semantic_logits is None:
            semantic_logits = torch.zeros(
                output_masks.shape[0],
                output_masks.shape[1],
                1,
                output_masks.shape[-2],
                output_masks.shape[-1],
                device=output_masks.device,
                dtype=output_masks.dtype,
            )
        return output_masks, iou_predictions, boundary_maps, semantic_logits
    ...
    @torch.jit.export
    def get_image_embeddings(self, batched_images: torch.Tensor) -> torch.Tensor:
        """
        Encode input images.

        Args:
          batched_images: [B, 3, H, W]
        """
    ...
    def forward(
        ...
        """
        Backward-compatible inference path returning masks and IoU predictions.
        """
    ...
    def forward_with_boundary(
        ...
        """
        Boundary-aware inference path returning masks, IoU, and boundaries.
        """
        _, _, input_h, input_w = batched_images.shape
        image_embeddings = self.get_image_embeddings(batched_images)
        return self.predict_masks_with_boundary(
            image_embeddings,
            batched_points,
            batched_point_labels,
            multimask_output=multimask_output,
    ...
    def forward_with_boundary_and_semantics(
        self,
        batched_images: torch.Tensor,
        batched_points: torch.Tensor,
        batched_point_labels: torch.Tensor,
        ...
        """
        Boundary-aware inference path returning masks, IoU, boundaries, and semantic logits.
        """
        _, _, input_h, input_w = batched_images.shape
        image_embeddings = self.get_image_embeddings(batched_images)
        return self.predict_masks_with_boundary_and_semantics(
            image_embeddings,
            batched_points,
            batched_point_labels,
            multimask_output=multimask_output,
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        """Normalize pixel values and resize to model input size."""
        if (
...
def build_efficient_sam(
    ...
    sam = EfficientSam(
        ...
        mask_decoder=MaskDecoder(
            ...
            enable_boundary_branch=enable_boundary_decoder,
    ...
    if checkpoint is not None:
        ...
        strict_load = not (
            enable_boundary_decoder or enable_semantic_head or semantic_num_classes > 1
        )
        sam.load_state_dict(model_state_dict, strict=strict_load)
    return sam
